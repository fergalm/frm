...
from .abstracttess import AbstractTess
...
class TessFitsPrf(AbstractTess):
    ...
    Notes
    -------
    There are so many TESS files, that this class handles downloading them from MAST
    on demand
    """
    ...
    def getBracketingPixelPositions(self, col:float, row:float) -> Tuple[Array1d, Array1d]:
    ...
    def getBracketingImages(self, brCol: Array1d, brRow: Array1d) -> List[SubSampledPrf]:
        fitsFileList = self.getFitsFilenames(brCol, brRow)
        subSampledImgList = self.loadFitsFiles(fitsFileList)
        return subSampledImgList

    def getFitsFilenames(self, brCol: Array1d, brRow: Array1d) -> List[str]:
...
def download(remoteUrl, local) -> None:
